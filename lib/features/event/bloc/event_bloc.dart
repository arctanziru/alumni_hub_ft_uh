...
class EventBloc extends Bloc<EventEvent, EventState> {
    ...
    return emit.forEach<InfiniteQueryState<EventGetManyModelResponse>>(
      ...
      return state.copyWith(
        status: queryState.status == QueryStatus.loading
            ...
            ? EventStatus.error
            : EventStatus.loaded,
        events: queryState.data?.expand((page) => page.data.data).toList() ?? [],
        error: CustomException(queryState.error?.message ?? 'Unknown error'),
      );
    });
  }

  FutureOr<void> _onEventRefreshed(EventRefreshed event, Emitter<EventState> emit) {
    if (event.isClear) {
      state.events.clear();
      state.search = null;
    }
  ...
  FutureOr<void> _onEventNextPage(EventNextPage event, Emitter<EventState> emit) {
    _eventRepository
        .getEvents(
      EventGetManyParams(
        search: state.search,
